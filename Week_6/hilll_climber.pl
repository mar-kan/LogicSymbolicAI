...
*/
     ...
     solve_hill_climb(Problem,State,_,[]) :-
	final_state(Problem,State).
     solve_hill_climb(Problem,State,History,[Move|Moves]) :-
	hill_climb(Problem,State,Move),
	update(State,Move,State1),
	legal(State1),
	\+ memberchk(State1,History),
	solve_hill_climb(Problem,State1,[State1|History],Moves).

     hill_climb(Problem,State,Move) :-
        findall(M,move(State,M),Moves),
        evaluate_and_order(Problem,Moves,State,[],MVs),
	member((Move,_),MVs).
...
*/
     evaluate_and_order(Problem,[Move|Moves],State,MVs,OrderedMVs) :-
	update(State,Move,State1),
	value(Problem,State1,Value),
	insert((Move,Value),MVs,MVs1),
	evaluate_and_order(Problem,Moves,State,MVs1,OrderedMVs).
     evaluate_and_order(_,[],_,MVs,MVs).
...
/*	Testing the Framework */
	...
	solve_hill_climb(Problem,Moves)  :-
		initial_state(Problem,State),
		solve_hill_climb(Problem,State,[State],Moves).
