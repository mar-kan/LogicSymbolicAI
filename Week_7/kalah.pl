...
/* Human choosing a move. */

choose_move(_,opponent,Move) :-
	repeat,
	nl,
	writeln('Please make a move by entering "[N].", where "N" is the number of the tray from which you want to play, counted from your left. If this move would allow you another move, enter "[M, N, ...].", a list of moves in order.'),
	read(Move),
	legal(Move).
...
move(board([0,0,0,0,0,0],_,_,_),[]).

stones_in_hole(M,board(Hs,_,_,_),Stones) :-
	nth1(M,Hs,Stones),
	Stones > 0.

extend_move(Stones,M,_,[]) :-
	Stones =\= (7-M) mod 13,
	!.
...
check_capture(N,Stones,Hs,Hs1,Ys,Ys1,Pieces) :-
	...
	nth1(FinishingHole,Hs,1),
	OppositeHole is 7-FinishingHole,
	nth1(OppositeHole,Ys,Y),
	Y > 0,
	!,
	replacenth1(OppositeHole,Ys,0,Ys1),
	replacenth1(FinishingHole,Hs,0,Hs1),
	Pieces is Y+1.
check_capture(_,_,Hs,Hs,Ys,Ys,0) :-
	!.
...
update_kalah(0,Stones,N,K,K) :-
	Stones < 7-N,
	!.
update_kalah(0,Stones,N,K,K1) :-
	Stones =:= 7-N,
	!,
	K1 is K+1.
update_kalah(Pieces,_,_,K,K1) :-
	Pieces > 0,
	!,
	K1 is K+Pieces.
...
distribute_your_holes(Stones,board(Hs,K,Ys,L),board(Hs,K,Ys1,L)) :-
	...
	distribute(6,Ys,Ys1),
	Stones1 is Stones-6,
	distribute_stones(Stones1,0,board(Hs,K,Ys1,L),_).
...
pick_up_and_distribute(0,N,Hs,Hs1) :-
	!,
	distribute(N,Hs,Hs1).
pick_up_and_distribute(1,N,[_|Hs],[0|Hs1]) :-
	!,
	distribute(N,Hs,Hs1).
pick_up_and_distribute(K,N,[H|Hs],[H|Hs1]) :-
	...
	!,
	K1 is K-1,
	pick_up_and_distribute(K1,N,Hs,Hs1).
...
game_over(board(0,N,0,N),_,draw) :-
	pieces(K),
	N =:= 6*K,
	!.
game_over(board(_,K,_,_),Player,Player) :-
	...
	K > 6*N,
	!.
game_over(board(_,_,_,L),Player,Opponent) :-
	pieces(N),
	L > 6*N,
